...
- Agent widgets above the chat input no longer disappear right after a turn: the bar stays with a hide chevron, and a small show chip brings it back (with a notice when there is nothing left to show). Hovering or tapping a settled widget opens a popup with the agent's collected JSON as a readable tree, its error when it failed, and — with the new per-chat Chat Settings → Agents → Capture agent reasoning toggle — a button to read the agent's reasoning. Any popup content can open fullscreen with a copy button, and the translation widget shows the turn's translated text the same way.
